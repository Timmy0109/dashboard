...
const recent = computed<Notification[]>(() =>
...
<template>
  ...
  >
    ...
    <v-card width="360" class="pms-bell-panel">
      <div class="d-flex align-center justify-space-between pa-3">
        <div class="text-subtitle-2 font-weight-semibold">通知</div>
        <v-btn
          v-if="unreadCount > 0"
          variant="text"
          size="small"
      ...
      <div v-if="recent.length === 0" class="py-4">
        <EmptyState
      ...
      <v-list v-else density="compact" nav class="py-1" max-height="420">
        <v-list-item
          v-for="n in recent"
          :key="n.id"
      ...
      <div class="pa-2 d-flex justify-center">
        <v-btn variant="text" size="small" block @click="onViewAll">
          查看全部
        </v-btn>
      </div>
    </v-card>
